...
net_devices <- function(x){
  encsizemap1=c('1'=4, '2'=8, '3'=8, '4'=8,'5'=8,'6'=12,'7'=0, '8'=0,'9'=0, '10'=4,'11'=8,'12'=12,'13'=12,'14'=12,'15'=12,'16'=32,'17'=64,'18'=96, '19'=288,'20'=32,'21'=64, '22'=8,'23'=4,'24'=8,'25'=32)
  ...
  x <-  tidyr::pivot_longer(x,c(label, label_2), names_to='lab', values_to='name') |>
    ...
    dplyr::mutate(Nout = dplyr::case_when(enc_type == '0' ~ 288,enc_type == '3' ~ 2,
...
net_virtual_cables <- function(x, directed=TRUE){
  y <- dplyr::filter(x, enc_type == 5, !is.na(label),label != " ", !is.na(label_2), label_2 != " ")
  t1 <- tibble::tibble(from=y$label, to=y$label_2)
  
  z <- dplyr::filter(x, enc_type == 6, !is.na(label))
  s <- dplyr::filter(x, enc_type == 7, !is.na(label) & label != " ")
  r <- dplyr::filter(x, enc_type == 7, !is.na(label_2) & label_2 !=" ")
  
  f <- function(x){
    if (length(x)>0) {
      # Reduce(dplyr::bind_rows, x) |>
      tidyr::pivot_longer(x, cols=starts_with("to"), values_to ='to') |> select(from,to) |> filter(!is.na(to))
    } else {tibble()}
  }
  
  
  t2 <- purrr::pmap_df(list(z[s,]$geom, z[s,]$label),(\(x,y) c(from=y,to=s[unlist(sf::st_is_within_distance(x,s,dist=0.1)),]$label)))
  t3 <- purrr::pmap(list(z[r,]$geom, z[r,]$label),(\(x,y) c(from=y,to=r[unlist(sf::st_is_within_distance(x,r,dist=0.1)),]$label_2)))
  
  
  t <- t1#dplyr::bind_rows(t1,t2,t3)
  t$ref <- paste0(t$from,'/', t$to)
...
net_boundaries <- function(b, directed=TRUE){
  ...
  lboundaries <- sf::st_within(b,sf::st_buffer(b,1)) #1 meter of buffer added to avoid geometries issues
...
#' Group sf by geometry
#'
#' @param sfx = layer in sf format
#'
#' @export
#' @return list of sf grouped by geom
...
#' Group sf by touch another grouped geometry
#'
#' @param groupedsfx = list of sf (result from net_geomgrouped) (from cables to closures)
#' @param sfy = layer in sf format
#'
#' @export
#' @return list of sf grouped by touch
net_groupedbytouch2 <- function(groupedsfx, sfy){
  #  groupedsfx |> lapply (\(x) sfy[x,op=sf::st_touches])
  #  groupedsfx |> lapply (\(x) sfy[unique(unlist(sf::st_is_within_distance(x,sfy,dist=0.1))),]|> (\(x) x[!st_is_empty(x),])())
  groupedsfx |>  lapply (\(x)
                         sfy[c(unique(unlist(sf::st_is_within_distance(lwgeom::st_startpoint(x),sfy,dist=0.1))),unique(unlist(sf::st_is_within_distance(lwgeom::st_endpoint(x),sfy,dist=0.1)))),] |>
                           (\(x) x[!st_is_empty(x),])())
}

#' Group sf by touch another grouped geometry
#'
#' @param groupedsfx = list of sf (result from net_geomgrouped) (from premise to cables)
#' @param sfy = layer in sf format
#'
#' @export
#' @return list of sf grouped by touch
net_groupedbytouch <- function(groupedsfx, sfy){
  ...
  #  groupedsfx |> lapply (\(x) sfy[unique(unlist(sf::st_is_within_distance(x,sfy,dist=0.1))),]|> (\(x) x[!st_is_empty(x),])())
  groupedsfx |>
    lapply (\(x) sfy[c(unique(unlist(sf::st_is_within_distance(x,lwgeom::st_endpoint(sfy), dist=0.1))),unique(unlist(sf::st_is_within_distance(x,lwgeom::st_startpoint(sfy), dist=0.1)))),] |>
              (\(x) x[!st_is_empty(x),])())
}
...
#' Group sf by touch another grouped geometry
#'
#' @param groupedsfx = list of sf (result from net_geomgrouped)
#' @param sfy = layer in sf formtat
#'
#' @export
#' @return list of sf grouped by intersect
net_groupedbyintersect <- function(groupedsfx, sfy){
  groupedsfx |> lapply (\(x) sfy[x,op=sf::st_intersects])
}
...
designed_prems <- function(g,cables, w= sf::st_sf(geometry = sf::st_sfc(sf::st_multipoint()))){
  x <- cables[cables$cab_size==7,]
  #  dp <- g[x,op=sf::st_touches] |> dplyr::bind_rows(w)
  dp <- g[c(unique(unlist(sf::st_is_within_distance(lwgeom::st_startpoint(x),g,dist=0.1))),unique(unlist(sf::st_is_within_distance(lwgeom::st_endpoint(x),g,dist=0.1)))),]|> dplyr::bind_rows(w)
  #dp <- g[unique(unlist(sf::st_is_within_distance(x,g,dist=0.1))),]|> dplyr::bind_rows(w)
  
  dp <- dp[!sf::st_is_empty(dp),]
  return (dp)
}
...
#' Create SLD plot
#'
#' @param g graph of tidygraph class
#' @param fname filename
#' @param layout type of layout
#'
#' @export
#' @return graph SLD plot
net_plot_sld <- function(x, dir, fname,layout='tree', h=1, w =1){
  
  tt <- function(name){
    #paste0(dir,"\\fas\\",name,'.svg')
    # paste0('<iframe
    #          width=\"500\"
    #          height=\"500\"
    #          src =\"', filefas,
    #        '\"></iframe>')
    #paste0('<img src=\"',filefas,'\" width=\"300" heigth="300"\">')
    #paste0(c(paste0(str_split(output_dir,'\\\\')[[1]],collapse='/'), 'fas',name), collapse='/')
    paste0('fas/',name,'.svg')
    #file.path(dir,'fas', paste0(name,'.svg'))
  }
  ...
  ggplot2::ggsave(plot=p, filename=file.path(dir, paste0(fname,'.svg')), device = 'svg', width = w*15*yr,height=h*3*xr+2,units="in", limitsize=FALSE)
  ####
  x <- ggiraph::girafe(ggobj = p,width_svg=120*w, height_svg=1.5*100*h/2)#, zoom_max=5)# width_svg=15*w*yr, height_svg=15*h*xr, zoom_max=5)
  x <- ggiraph::girafe_options(x, ggiraph::opts_zoom(min = .7, max = 1000) )
  #    if( interactive() ) htmltools::save_html(x, file.path(dir, paste0('sldfas_',fname,'.html')))
...
#' #' @export
#' #' @return graph Network preliminary plot
#' net_plot_b <- function(x, name='temp.svg',directory=paste0(getwd(),'/boundaries'),layout='tree'){
#'   layout <- ggraph::create_layout(x, layout)
#'   layout$y <- -layout$y
#'   yr <- max(sapply(unique(layout$x), (\(k) length(layout$y[layout$x==k]))))
#'   xr <- max(sapply(unique(layout$y), (\(k) length(layout$x[layout$y==k]))))
#'   p <- ggraph::ggraph(layout) +
#'     ggraph::geom_edge_diagonal(aes(colour=as.factor(warn)),width = 0.5,alpha=0.5) +
#'     ggraph::geom_node_label(aes(label = label_text,colour = as.factor(level_), fill=alert), repel=FALSE)+
#'     ggplot2::scale_colour_manual(values=c("2"="#126b10", "3"="#b81707", "5"="orange","8"="pink","9"="green","1"="black", "99"="red", "0"="grey"))+
